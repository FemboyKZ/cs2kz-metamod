...
KZClassicNopreModePlugin g_KZClassicNopreModePlugin;

CGameConfig *g_pGameConfig = NULL;
KZUtils *g_pKZUtils = NULL;
KZModeManager *g_pModeManager = NULL;
MappingInterface *g_pMappingApi = NULL;
ModeServiceFactory g_ModeFactory = [](KZPlayer *player) -> KZModeService * { return new KZClassicNopreModeService(player); };
PLUGIN_EXPOSE(KZClassicNopreModePlugin, g_KZClassicNopreModePlugin);

CConVarRef<f32> sv_standable_normal("sv_standable_normal");

bool KZClassicNopreModePlugin::Load(PluginId id, ISmmAPI *ismm, char *error, size_t maxlen, bool late)
{
	PLUGIN_SAVEVARS();
	// Load mode
	int success;
...
bool KZClassicNopreModePlugin::Unload(char *error, size_t maxlen)
{
	g_pModeManager->UnregisterMode(g_PLID);
	return true;
}

bool KZClassicNopreModePlugin::Pause(char *error, size_t maxlen)
{
	g_pModeManager->UnregisterMode(g_PLID);
	return true;
}

bool KZClassicNopreModePlugin::Unpause(char *error, size_t maxlen)
{
	if (!g_pModeManager->RegisterMode(g_PLID, MODE_NAME_SHORT, MODE_NAME, g_ModeFactory))
	{
		return false;
...
void KZClassicNopreModeService::Reset()
{
...
void KZClassicNopreModeService::Cleanup()
{
...
const char *KZClassicNopreModeService::GetModeName()
{
	return MODE_NAME;
}

const char *KZClassicNopreModeService::GetModeShortName()
{
	return MODE_NAME_SHORT;
}

bool KZClassicNopreModeService::EnableWaterFix()
{
	return this->player->IsButtonPressed(IN_JUMP);
}

DistanceTier KZClassicNopreModeService::GetDistanceTier(JumpType jumpType, f32 distance)
{
...
const CVValue_t *KZClassicNopreModeService::GetModeConVarValues()
{
	return modeCvarValues;
}

void KZClassicNopreModeService::OnStopTouchGround()
{
	Vector velocity;
...
void KZClassicNopreModeService::OnStartTouchGround()
{
	this->SlopeFix();
	bbox_t bounds;
...
void KZClassicNopreModeService::OnPhysicsSimulate()
{
...
void KZClassicNopreModeService::OnPhysicsSimulatePost()
{
	CCSPlayer_MovementServices *moveServices = this->player->GetMoveServices();
	if (!moveServices)
	{
...
void KZClassicNopreModeService::OnSetupMove(PlayerCommand *pc)
{
...
void KZClassicNopreModeService::OnProcessMovement()
{
	this->didTPM = false;
...
void KZClassicNopreModeService::OnProcessMovementPost()
{
	...
	f32 velMod = this->originalMaxSpeed >= 0 ? SPEED_NORMAL / this->originalMaxSpeed : 1.0f;
...
void KZClassicNopreModeService::InterpolateViewAngles()
{
...
void KZClassicNopreModeService::RestoreInterpolatedViewAngles()
{
...
void KZClassicNopreModeService::RemoveCrouchJumpBind()
{
...
void KZClassicNopreModeService::ReduceDuckSlowdown()
{
...
void KZClassicNopreModeService::UpdateAngleHistory()
{
...
void KZClassicNopreModeService::CalcPrestrafe()
{
	f32 totalDuration = 0;
...
f32 KZClassicNopreModeService::GetPrestrafeGain()
{
	return PS_SPEED_MAX * pow(MAX(this->leftPreRatio, this->rightPreRatio) / PS_MAX_PS_TIME, PS_RATIO_TO_SPEED);
}

void KZClassicNopreModeService::CheckVelocityQuantization()
{
...
void KZClassicNopreModeService::SlopeFix()
{
...
void KZClassicNopreModeService::OnTryPlayerMove(Vector *pFirstDest, trace_t *pFirstTrace, bool *bIsSurfing)
{
	this->tpmTriggerFixOrigins.RemoveAll();
...
void KZClassicNopreModeService::OnTryPlayerMovePost(Vector *pFirstDest, trace_t *pFirstTrace, bool *bIsSurfing)
{
	Vector velocity;
	this->player->GetVelocity(&velocity);
...
void KZClassicNopreModeService::OnCategorizePosition(bool bStayOnGround)
{
...
void KZClassicNopreModeService::OnDuckPost()
{
	this->player->UpdateTriggerTouchList();
}

void KZClassicNopreModeService::OnAirMove()
{
	this->airMoving = true;
	this->player->currentMoveData->m_flMaxSpeed = SPEED_NORMAL;
}

void KZClassicNopreModeService::OnAirMovePost()
{
	this->airMoving = false;
	this->player->currentMoveData->m_flMaxSpeed = SPEED_NORMAL;
}

void KZClassicNopreModeService::OnWaterMove()
{
	this->player->currentMoveData->m_flMaxSpeed = SPEED_NORMAL;
}

void KZClassicNopreModeService::OnWaterMovePost()
{
	this->player->currentMoveData->m_flMaxSpeed = SPEED_NORMAL;
}

void KZClassicNopreModeService::OnTeleport(const Vector *newPosition, const QAngle *newAngles, const Vector *newVelocity)
{
...
bool KZClassicNopreModeService::OnTriggerStartTouch(CBaseTrigger *trigger)
{
...
bool KZClassicNopreModeService::OnTriggerTouch(CBaseTrigger *trigger)
{
	if (!g_pMappingApi->IsTriggerATimerZone(trigger))
	{
		return true;
...
bool KZClassicNopreModeService::OnTriggerEndTouch(CBaseTrigger *trigger)
{
	if (!g_pMappingApi->IsTriggerATimerZone(trigger))
	{
		return true;
