...
KZNoPreModePlugin g_KZNoPreModePlugin;

CGameConfig *g_pGameConfig = NULL;
KZUtils *g_pKZUtils = NULL;
KZModeManager *g_pModeManager = NULL;
ModeServiceFactory g_ModeFactory = [](KZPlayer *player) -> KZModeService * { return new KZNoPreModeService(player); };
PLUGIN_EXPOSE(KZNoPreModePlugin, g_KZNoPreModePlugin);

bool KZNoPreModePlugin::Load(PluginId id, ISmmAPI *ismm, char *error, size_t maxlen, bool late)
{
	PLUGIN_SAVEVARS();
	// Load mode
	int success;
...
bool KZNoPreModePlugin::Unload(char *error, size_t maxlen)
{
	g_pModeManager->UnregisterMode(MODE_NAME);
	return true;
}

bool KZNoPreModePlugin::Pause(char *error, size_t maxlen)
{
	g_pModeManager->UnregisterMode(MODE_NAME);
	return true;
}

bool KZNoPreModePlugin::Unpause(char *error, size_t maxlen)
{
	if (!g_pModeManager->RegisterMode(g_PLID, MODE_NAME_SHORT, MODE_NAME, g_ModeFactory))
	{
		return false;
...
const char *KZNoPreModePlugin::GetLicense()
{
	return "GPLv3";
}

const char *KZNoPreModePlugin::GetVersion()
{
	return VERSION_STRING;
}

const char *KZNoPreModePlugin::GetDate()
{
	return __DATE__;
}

const char *KZNoPreModePlugin::GetLogTag()
{
	return "KZ-Mode-Classic-Bind";
}

const char *KZNoPreModePlugin::GetAuthor()
{
	return "zer0.k - (Dots mod :3)";
}

const char *KZNoPreModePlugin::GetDescription()
{
	return "Classic-Bind mode plugin for CS2KZ";
}

const char *KZNoPreModePlugin::GetName()
{
	return "CS2KZ-Mode-Classic-Bind";
}

const char *KZNoPreModePlugin::GetURL()
{
...
void KZNoPreModeService::Cleanup()
{
...
const char *KZNoPreModeService::GetModeName()
{
	return MODE_NAME;
}

const char *KZNoPreModeService::GetModeShortName()
{
	return MODE_NAME_SHORT;
}

bool KZNoPreModeService::EnableWaterFix()
{
	return this->player->IsButtonPressed(IN_JUMP);
}

DistanceTier KZNoPreModeService::GetDistanceTier(JumpType jumpType, f32 distance)
{
...
META_RES KZNoPreModeService::GetPlayerMaxSpeed(f32 &maxSpeed)
{
	...
	this->tweakedMaxSpeed = SPEED_NORMAL;
...
void KZNoPreModeService::OnJump()
{
	Vector velocity;
...
void KZNoPreModeService::OnJumpPost()
{
...
void KZNoPreModeService::OnStopTouchGround()
{
	Vector velocity;
	this->player->GetVelocity(&velocity);
...
void KZNoPreModeService::OnStartTouchGround()
{
	this->SlopeFix();
	bbox_t bounds;
...
void KZNoPreModeService::OnPhysicsSimulate()
{
	this->InsertSubtickTiming(g_pKZUtils->GetServerGlobals()->tickcount * ENGINE_FIXED_TICK_INTERVAL - 0.5 * ENGINE_FIXED_TICK_INTERVAL);
}

void KZNoPreModeService::OnPhysicsSimulatePost()
{
	this->InsertSubtickTiming(g_pKZUtils->GetServerGlobals()->tickcount * ENGINE_FIXED_TICK_INTERVAL + 0.5 * ENGINE_FIXED_TICK_INTERVAL);
}

void KZNoPreModeService::OnProcessUsercmds(void *cmds, int numcmds)
{
...
void KZNoPreModeService::OnProcessMovement()
{
	this->didTPM = false;
...
void KZNoPreModeService::OnProcessMovementPost()
{
...
void KZNoPreModeService::InsertSubtickTiming(float time)
{
...
void KZNoPreModeService::InterpolateViewAngles()
{
...
void KZNoPreModeService::RestoreInterpolatedViewAngles()
{
...
void KZNoPreModeService::SlopeFix()
{
...
void KZNoPreModeService::OnTryPlayerMove(Vector *pFirstDest, trace_t *pFirstTrace)
{
	this->tpmTriggerFixOrigins.RemoveAll();
...
void KZNoPreModeService::OnTryPlayerMovePost(Vector *pFirstDest, trace_t *pFirstTrace)
{
	Vector velocity;
	this->player->GetVelocity(&velocity);
...
void KZNoPreModeService::OnCategorizePosition(bool bStayOnGround)
{
...
void KZNoPreModeService::OnDuckPost()
{
	this->player->UpdateTriggerTouchList();
}

void KZNoPreModeService::OnAirMove()
{
	this->airMoving = true;
}

void KZNoPreModeService::OnAirMovePost()
{
	this->airMoving = false;
}

void KZNoPreModeService::OnTeleport(const Vector *newPosition, const QAngle *newAngles, const Vector *newVelocity)
{
...
bool KZNoPreModeService::OnTriggerStartTouch(CBaseTrigger *trigger)
{
...
bool KZNoPreModeService::OnTriggerTouch(CBaseTrigger *trigger)
{
	if (!trigger->IsEndZone() && !trigger->IsStartZone())
	{
		return true;
...
bool KZNoPreModeService::OnTriggerEndTouch(CBaseTrigger *trigger)
{
