...
bool KZBhopModePlugin::Unload(char *error, size_t maxlen)
{
	g_pModeManager->UnregisterMode(MODE_NAME);
	return true;
}

bool KZBhopModePlugin::Pause(char *error, size_t maxlen)
{
	g_pModeManager->UnregisterMode(MODE_NAME);
	return true;
}

bool KZBhopModePlugin::Unpause(char *error, size_t maxlen)
{
	if (!g_pModeManager->RegisterMode(g_PLID, MODE_NAME_SHORT, MODE_NAME, g_ModeFactory))
	{
		return false;
...
const char *KZBhopModePlugin::GetLicense()
{
	return "GPLv3";
}

const char *KZBhopModePlugin::GetVersion()
{
	return VERSION_STRING;
}

const char *KZBhopModePlugin::GetDate()
{
	return __DATE__;
}

const char *KZBhopModePlugin::GetLogTag()
{
	return "KZ-Mode-Bhop";
}

const char *KZBhopModePlugin::GetAuthor()
{
	return "zer0.k - (Dots mod :3)";
}

const char *KZBhopModePlugin::GetDescription()
{
	return "Bhop mode plugin for CS2KZ";
}

const char *KZBhopModePlugin::GetName()
{
	return "CS2KZ-Mode-Bhop";
}

const char *KZBhopModePlugin::GetURL()
{
...
const char *KZBhopModeService::GetModeShortName()
{
	return MODE_NAME_SHORT;
}

bool KZBhopModeService::EnableWaterFix()
{
	return this->player->IsButtonPressed(IN_JUMP);
}

DistanceTier KZBhopModeService::GetDistanceTier(JumpType jumpType, f32 distance)
{
...
const char **KZBhopModeService::GetModeConVarValues()
{
	return modeCvarValues;
}

// Attempt to replicate 128t jump height.
void KZBhopModeService::OnJump()
{
	Vector velocity;
...
void KZBhopModeService::OnJumpPost()
{
...
void KZBhopModeService::OnStopTouchGround()
{
	Vector velocity;
	this->player->GetVelocity(&velocity);
...
void KZBhopModeService::OnStartTouchGround()
{
	this->SlopeFix();
	bbox_t bounds;
...
void KZBhopModeService::OnPhysicsSimulate()
{
	this->InsertSubtickTiming(g_pKZUtils->GetServerGlobals()->tickcount * ENGINE_FIXED_TICK_INTERVAL - 0.5 * ENGINE_FIXED_TICK_INTERVAL);
}

void KZBhopModeService::OnPhysicsSimulatePost()
{
	this->InsertSubtickTiming(g_pKZUtils->GetServerGlobals()->tickcount * ENGINE_FIXED_TICK_INTERVAL + 0.5 * ENGINE_FIXED_TICK_INTERVAL);
}

void KZBhopModeService::OnProcessUsercmds(void *cmds, int numcmds)
{
...
void KZBhopModeService::OnProcessMovementPost()
{
...
void KZBhopModeService::InsertSubtickTiming(float time)
{
...
void KZBhopModeService::InterpolateViewAngles()
{
...
void KZBhopModeService::RestoreInterpolatedViewAngles()
{
...
/*
void KZBhopModeService::RemoveCrouchJumpBind()
{
...
void KZBhopModeService::ReduceDuckSlowdown()
{
...
void KZBhopModeService::UpdateAngleHistory()
{
...
void KZBhopModeService::CalcPrestrafe()
{
	f32 totalDuration = 0;
...
void KZBhopModeService::SlopeFix()
{
...
void KZBhopModeService::OnTryPlayerMove(Vector *pFirstDest, trace_t *pFirstTrace)
{
	this->tpmTriggerFixOrigins.RemoveAll();
...
void KZBhopModeService::OnTryPlayerMovePost(Vector *pFirstDest, trace_t *pFirstTrace)
{
	Vector velocity;
	this->player->GetVelocity(&velocity);
...
void KZBhopModeService::OnCategorizePosition(bool bStayOnGround)
{
...
void KZBhopModeService::OnDuckPost()
{
	this->player->UpdateTriggerTouchList();
}
