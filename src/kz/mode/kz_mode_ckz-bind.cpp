...
bool KZClassicBindModePlugin::Unload(char *error, size_t maxlen)
{
	g_pModeManager->UnregisterMode(MODE_NAME);
	return true;
}

bool KZClassicBindModePlugin::Pause(char *error, size_t maxlen)
{
	g_pModeManager->UnregisterMode(MODE_NAME);
	return true;
}

bool KZClassicBindModePlugin::Unpause(char *error, size_t maxlen)
{
	if (!g_pModeManager->RegisterMode(g_PLID, MODE_NAME_SHORT, MODE_NAME, g_ModeFactory))
	{
		return false;
...
const char *KZClassicBindModePlugin::GetLicense()
{
	return "GPLv3";
}

const char *KZClassicBindModePlugin::GetVersion()
{
	return VERSION_STRING;
}

const char *KZClassicBindModePlugin::GetDate()
{
	return __DATE__;
}

const char *KZClassicBindModePlugin::GetLogTag()
{
	return "KZ-Mode-Classic-Bind";
}

const char *KZClassicBindModePlugin::GetAuthor()
{
	return "zer0.k - (Dots mod :3)";
}

const char *KZClassicBindModePlugin::GetDescription()
{
	return "Classic-Bind mode plugin for CS2KZ";
}

const char *KZClassicBindModePlugin::GetName()
{
	return "CS2KZ-Mode-Classic-Bind";
}

const char *KZClassicBindModePlugin::GetURL()
{
	return "https://github.com/FemboyKZ/cs2kz-metamod";
}
...
const char *KZClassicBindModeService::GetModeShortName()
{
	return MODE_NAME_SHORT;
}

bool KZClassicBindModeService::EnableWaterFix()
{
	return this->player->IsButtonPressed(IN_JUMP);
}

DistanceTier KZClassicBindModeService::GetDistanceTier(JumpType jumpType, f32 distance)
{
...
const char **KZClassicBindModeService::GetModeConVarValues()
{
...
void KZClassicBindModeService::OnStartTouchGround()
{
	this->SlopeFix();
	bbox_t bounds;
...
void KZClassicBindModeService::OnPhysicsSimulate()
{
	this->InsertSubtickTiming(g_pKZUtils->GetServerGlobals()->tickcount * ENGINE_FIXED_TICK_INTERVAL - 0.5 * ENGINE_FIXED_TICK_INTERVAL);
}

void KZClassicBindModeService::OnPhysicsSimulatePost()
{
	this->InsertSubtickTiming(g_pKZUtils->GetServerGlobals()->tickcount * ENGINE_FIXED_TICK_INTERVAL + 0.5 * ENGINE_FIXED_TICK_INTERVAL);
}

void KZClassicBindModeService::OnSetupMove(PlayerCommand *pc)
{
...
void KZClassicBindModeService::OnProcessMovementPost()
{
...
void KZClassicBindModeService::InsertSubtickTiming(float time)
{
...
void KZClassicBindModeService::InterpolateViewAngles()
{
...
void KZClassicBindModeService::RestoreInterpolatedViewAngles()
{
...
/*
void KZClassicBindModeService::RemoveCrouchJumpBind()
{
...
void KZClassicBindModeService::ReduceDuckSlowdown()
{
...
void KZClassicBindModeService::UpdateAngleHistory()
{
...
void KZClassicBindModeService::CalcPrestrafe()
{
	f32 totalDuration = 0;
...
f32 KZClassicBindModeService::GetPrestrafeGain()
{
	return PS_SPEED_MAX * pow(MAX(this->leftPreRatio, this->rightPreRatio) / PS_MAX_PS_TIME, PS_RATIO_TO_SPEED);
}

void KZClassicBindModeService::CheckVelocityQuantization()
{
...
void KZClassicBindModeService::SlopeFix()
{
...
void KZClassicBindModeService::OnTryPlayerMove(Vector *pFirstDest, trace_t *pFirstTrace)
{
	this->tpmTriggerFixOrigins.RemoveAll();
...
void KZClassicBindModeService::OnTryPlayerMovePost(Vector *pFirstDest, trace_t *pFirstTrace)
{
	Vector velocity;
	this->player->GetVelocity(&velocity);
...
void KZClassicBindModeService::OnCategorizePosition(bool bStayOnGround)
{
...
void KZClassicBindModeService::OnDuckPost()
{
	this->player->UpdateTriggerTouchList();
}

void KZClassicBindModeService::OnAirMove()
{
	this->airMoving = true;
	this->player->currentMoveData->m_flMaxSpeed = SPEED_NORMAL;
}

void KZClassicBindModeService::OnAirMovePost()
{
	this->airMoving = false;
	this->player->currentMoveData->m_flMaxSpeed = SPEED_NORMAL + this->GetPrestrafeGain();
}

void KZClassicBindModeService::OnWaterMove()
{
	this->player->currentMoveData->m_flMaxSpeed = SPEED_NORMAL;
}

void KZClassicBindModeService::OnWaterMovePost()
{
	this->player->currentMoveData->m_flMaxSpeed = SPEED_NORMAL + this->GetPrestrafeGain();
}

void KZClassicBindModeService::OnTeleport(const Vector *newPosition, const QAngle *newAngles, const Vector *newVelocity)
{
...
bool KZClassicBindModeService::OnTriggerStartTouch(CBaseTrigger *trigger)
{
...
bool KZClassicBindModeService::OnTriggerTouch(CBaseTrigger *trigger)
{
	if (!g_pMappingApi->IsTriggerATimerZone(trigger))
	{
		return true;
...
bool KZClassicBindModeService::OnTriggerEndTouch(CBaseTrigger *trigger)
{
	if (!g_pMappingApi->IsTriggerATimerZone(trigger))
	{
		return true;
